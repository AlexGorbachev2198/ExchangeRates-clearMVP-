...
public class StateProgressBar extends View {
    ...
    public void setCurrentStateNumber(StateNumber currentStateNumber) {
        setCurrentState(currentStateNumber.getValue());
    }
    ...
    public void setCurrentState(int state) {
        ...
        if (mCurrentStateNumber > state) {
            mStartCenterX = mEndCenterX = 0;
        }
        this.mCurrentStateNumber = state;
        updateCheckAllStatesValues(mEnableAllStatesCompleted);
        invalidate();
    }

    public StateNumber getCurrentState() {
        StateNumber result;
        switch (getCurrentStateNumber()) {
            case (1):
                result = StateNumber.ONE;
                break;
            case (2):
                result = StateNumber.TWO;
                break;
            case (3):
                result = StateNumber.THREE;
                break;
            case (4):
                result = StateNumber.FOUR;
                break;
            case (5):
                result = StateNumber.FIVE;
                break;
            default:
                result = StateNumber.ONE;
        }
    ...
    public void setMaxStateNumber(StateNumber maximumState) {
        setMaxStateCount(maximumState.getValue());
    }

    public void setMaxStateCount(int count) {
        this.mMaxStateNumber = count;
        validateStateNumber(mCurrentStateNumber);
        updateCheckAllStatesValues(mEnableAllStatesCompleted);
        invalidate();
    }
    ...
    private void initStateProgressBar(Context context) {
        ...
        mBackgroundColor = ContextCompat.getColor(context, R.color.state_progress_bg_color);
        mForegroundColor = ContextCompat.getColor(context, R.color.state_progress_fg_color);
        mStateNumberBackgroundColor = ContextCompat.getColor(context, R.color.state_progress_number_bg_color);
        mStateNumberForegroundColor = ContextCompat.getColor(context, R.color.state_progress_number_fg_color);
        mCurrentStateDescriptionColor = ContextCompat.getColor(context, R.color.state_progress_fg_color);
        mStateDescriptionColor = ContextCompat.getColor(context, R.color.state_progress_fg_desc_color);
